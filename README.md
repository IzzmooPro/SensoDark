...
[![Version](https://img.shields.io/badge/version-1.7.2-7c5cfc)](https://github.com/IzzmooPro/SensoDark/releases)
...
- 🕵️ **Detects real dark themes first.** Before touching anything, SensoDark samples the page. If you're already on Reddit, YouTube, or any site with a genuine dark theme, it does nothing — no double-darkening, no fighting the site's own styles.
...
- 🖱️ **Hover states are covered.** Menu items and icon buttons that only turn light on `:hover` are caught the moment the pointer arrives, before the browser paints the hover state.
...
| 🎚️ **Per-site control** | Disable a site, mark it "already dark," or re-run detection with one click |
| 🤖 **Automation** | Manual, follows your OS theme, or runs on a schedule |
| ☁️ **Synced settings** | Stored via `chrome.storage.sync`; export/import from the options page |
| ⌨️ **Keyboard shortcuts** | `Alt+Shift+D` toggles globally, `Alt+Shift+A` toggles the current site |
...
- 🕵️ **Önce gerçek koyu temaları tanır.** Herhangi bir şeye dokunmadan önce sayfadan örnek alır. Zaten Reddit, YouTube gibi gerçek bir koyu temaya sahip bir sitedeyseniz hiçbir şey yapmaz — çifte karartma yok, sitenin kendi stilleriyle çakışma yok.
...
- 🖱️ **Hover durumları kapsanır.** Yalnızca `:hover` ile açık renge dönen menü öğeleri ve ikon butonları, imleç oraya geldiği anda, tarayıcı hover durumunu ekrana yansıtmadan önce yakalanır.
...
| 🎚️ **Site bazlı kontrol** | Siteyi devre dışı bırakma, "zaten koyu" işaretleme, tek tıkla yeniden algılama |
| 🤖 **Otomasyon** | Manuel, işletim sisteminin temasını takip eder, ya da bir zaman aralığında çalışır |
| ☁️ **Senkron ayarlar** | `chrome.storage.sync` ile saklanır; seçenekler sayfasından dışa/içe aktarılır |
| ⌨️ **Klavye kısayolları** | `Alt+Shift+D` genel aç/kapat, `Alt+Shift+A` bu site için aç/kapat |
...
[MIT](LICENSE) — telif bildirimini koruduğunuz sürece istediğinizi yapabilirsiniz.
